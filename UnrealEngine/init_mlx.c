...
/*   Updated: 2026/05/02 18:01:19 by aarab            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	load_tex(t_cube *cube, t_text *tex, char *path, char *err)
{
	int	width;
	int	height;
...
void	init_texture(t_cube *cube)
{
	...
	load_tex(cube, &cube->texture_hill, cube->map.no_t, "Error\nlol");
	load_tex(cube, &cube->sonic, cube->map.so_t, "Error\nlol2");
	load_tex(cube, &cube->bendi, cube->map.ea_t, "Error\nlol3");
	load_tex(cube, &cube->chaos, cube->map.we_t, "Error\nlol4");
}
